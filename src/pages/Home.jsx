...
const Home = () => {
	...
	// dataLoading es true si loading es true O si el catálogo global aún está vacío
	const dataLoading = loading || products.length === 0;
			...
			<main className="w-full min-h-screen pt-16 pb-12 flex flex-col items-center">
				...
				<section className="w-240 min-h-100 h-100 m-4 bg-(--steel-100) rounded-md shadow-md overflow-hidden p-4 flex items-center justify-between gap-4 relative">
					...
					<div className="flex flex-col gap-1 pl-8 pr-16">
						...
						{dataLoading ? <SkeletonLoader type="text" /> : <p className="uppercase font-medium text-xl">{products[0].nombre}</p>}
						{dataLoading ? <SkeletonLoader type="text" /> : <p className="font-medium">${formatNumberForDisplay(products[0].precio_unitario)}</p>}
						{dataLoading ? <SkeletonLoader type="text" /> : <p className="">Contamos con stock: {products[0].stock_actual}</p>}
				...
				<section className="w-240 min-h-80 h-80 m-4 bg-(--steel-100) rounded-md shadow-md overflow-hidden p-4 flex items-center justify-between gap-4 relative">
					{/* SLOT 1 */}
					{dataLoading 
                        ? <SkeletonLoader type="product-card-home" /> 
                        : cameras[0] ? <CardHome data={cameras[0]} /> : null
					}
					{/* SLOT 2 */}
					{dataLoading 
                        ? <SkeletonLoader type="product-card-home" /> 
                        : cameras[1] ? <CardHome data={cameras[1]} /> : null
					}
					{/* SLOT 3 */}
					{dataLoading 
                        ? <SkeletonLoader type="product-card-home" /> 
                        : cameras[2] ? <CardHome data={cameras[2]} /> : null
					}
					{/* SLOT 4 */}
					{dataLoading 
                        ? <SkeletonLoader type="product-card-home" /> 
                        : cameras[3] ? <CardHome data={cameras[3]} /> : null
					}
				</section>
				
				{/* -------------------- SECCIÓN ACCESORIOS DESTACADOS (products[8]) -------------------- */}
				<section className="w-240 min-h-100 h-100 m-4 bg-(--steel-100) rounded-md shadow-md overflow-hidden p-4 flex items-center justify-between gap-4 relative">
					<div className="bg-white size-92 min-w-92 flex items-center justify-center p-12 rounded-md">
						{
							dataLoading ?
					...
					<div className="flex flex-col gap-1 pl-8 pr-16">
						...
						{dataLoading ? <SkeletonLoader type="text" /> : <p className="uppercase font-medium text-xl">{products[8].nombre}</p>}
						{dataLoading ? <SkeletonLoader type="text" /> : <p className="font-medium">${formatNumberForDisplay(products[8].precio_unitario)}</p>}
						{dataLoading ? <SkeletonLoader type="text" /> : <p className="">Contamos con stock: {products[8].stock_actual}</p>}
				...
				{/* -------------------- SECCIÓN ACCESORIOS (ÍNDICES FIJOS) -------------------- */}
				<h2 className="w-240 px-2 py-5 text-lg uppercase">Accesorios</h2>
				<section className="w-240 min-h-80 h-80 m-4 bg-(--steel-100) rounded-md shadow-md overflow-hidden p-4 flex items-center justify-between gap-4 relative">
					{/* SLOT 1 */}
					{dataLoading 
                        ? <SkeletonLoader type="product-card-home" /> 
                        : accessories[0] ? <CardHome data={accessories[0]} /> : null
					}
					{/* SLOT 2 */}
					{dataLoading 
                        ? <SkeletonLoader type="product-card-home" /> 
                        : accessories[1] ? <CardHome data={accessories[1]} /> : null
					}
					{/* SLOT 3 */}
					{dataLoading 
                        ? <SkeletonLoader type="product-card-home" /> 
                        : accessories[2] ? <CardHome data={accessories[2]} /> : null
					}
					{/* SLOT 4 */}
					{dataLoading 
                        ? <SkeletonLoader type="product-card-home" /> 
                        : accessories[3] ? <CardHome data={accessories[3]} /> : null
					}
				</section>
			</main>
			<Footer/>
